...
def get_node_from_ip(ip_string):
    """Get a Node object form the IP address string"""
    try:
        dhcp_lease = DHCPLease.objects.get(ip=ip_string)
        mac_obj = MACAddress.objects.get(mac_address=dhcp_lease.mac)
        node = mac_obj.node
    except:
        node = None
    return node
...
def pxeconfig(request):
    ...
    if not mac:
        # This requires that you use the built in DHCP server
        ip_from_req = request.GET.get('remote', None)
        node = get_node_from_ip(ip_from_req)
    else:
        node = get_node_from_mac_string(mac)
